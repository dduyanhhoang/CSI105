...
% The paper headers
\markboth{CSI105 Report, Febuary~2024}%
...
\begin{center}
\begin{tikzpicture}[squarenode/.style={rectangle,rounded corners,draw=black, thin,minimum size = 5mm, align=center }]
%Nodes
\node[squarenode](maintopic){
    \scriptsize Function: main\\
    \scriptsize type: void
} ;
\node[squarenode](lower) [below=0.55cm of maintopic]{
    \scriptsize statement\_list
};
\node[](lowersub) [below=0.55cm of lower] {}                           ;
\node[squarenode](right) [right=0.9cm of lowersub] {
    \scriptsize call\_expr\\
    \scriptsize fn: printf
};
\node[squarenode](left)  [left=0.9cm of lowersub]{
    \scriptsize return\_expr\\
};
\node[squarenode] (leftsub)[below=0.55cm of left]{
    \scriptsize string\_cst\\
    \scriptsize strg: "Hello, World!"
};
\node[squarenode] (rightsub)[below=0.55cm of right]  {
    \scriptsize integer\_cst\\
    \scriptsize int: 0
};

%Lines
\draw[->] (maintopic.south) -- node[auto,swap] {...} (lower.north);
\draw[->] (lower.south) -- node[auto] {...}          (right.north);
\draw[->] (lower.south) -- node[auto,swap] {...}     (left.north);
\draw[->] (left.south) -- node[auto,swap] {...}      (leftsub.north);
\draw[->] (right.south) -- node[auto] {...}          (rightsub.north);
\end{tikzpicture}
\end{center}
